...
<li><strong>Essotherm650</strong>, a medium model for thermal oil, also based on tables.</li>
  ...
  package TableBased "Incompressible medium properties based on tables"
    import Poly = ThermofluidStream.myMedia.Incompressible.TableBased.Polynomials_Temp;

    extends myMedia.Interfaces.PartialMedium(
      ThermoStates=if enthalpyOfT then myMedia.Interfaces.Choices.IndependentVariables.T
           else myMedia.Interfaces.Choices.IndependentVariables.pT,
      final reducedX=true,
      final fixedX=true,
