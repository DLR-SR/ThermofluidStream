...
package MoistAir "Air: Moist air model (190 ... 647 K)"
  ...
  import ThermofluidStream.myMedia.IdealGases.Common.Functions;
  ...
  import ThermofluidStream.myMedia.Interfaces.Choices.ReferenceEnthalpy;
...
Specific enthalpy of liquid water is computed from temperature using a polynomial approach. Kept for compatibility reasons, better use <a href=\"modelica://Modelica.Media.Air.MoistAir.enthalpyOfWater\">enthalpyOfWater</a> instead.
  ...
  algorithm
    h := myMedia.IdealGases.Common.Functions.h_Tlow(
      ...
      myMedia.IdealGases.Common.Functions.h_Tlow(
        data=dryair,
        T=T,
        refChoice=ReferenceEnthalpy.UserDefined,
...
Specific enthalpy of moist air is computed from pressure, temperature and composition with X[1] as the total water mass fraction. The fog region is included for both, ice and liquid fog.
  ...
  algorithm
    ...
    h_der := X_steam*myMedia.IdealGases.Common.Functions.h_Tlow_der(
      ...
      myMedia.IdealGases.Common.Functions.h_Tlow_der(
        data=dryair,
        T=T,
        refChoice=ReferenceEnthalpy.UserDefined,
        h_off=25104.684,
...
The specific heat capacity at constant pressure <strong>cp</strong> is computed from temperature and composition for a mixture of steam (X[1]) and dry air. All water is assumed to be in the vapor state.
  ...
  algorithm
    cv := myMedia.IdealGases.Common.Functions.cp_Tlow(dryair, state.T)*(1 -
      state.X[Water]) + myMedia.IdealGases.Common.Functions.cp_Tlow(steam,
...
redeclare function extends dynamicViscosity
  ...
  import ThermofluidStream.myMedia.Incompressible.TableBased.Polynomials_Temp;
algorithm
...
<p>2012-01-12        Stefan Wischhusen: Initial Release.</p>
  ...
  algorithm
    s := myMedia.IdealGases.Common.Functions.s0_Tlow(dryair, T)*(1 - X[Water])
       + myMedia.IdealGases.Common.Functions.s0_Tlow(steam, T)*X[Water] -
...
<p>2012-01-12        Stefan Wischhusen: Initial Release.</p>
  ...
  algorithm
    ds := myMedia.IdealGases.Common.Functions.s0_Tlow_der(
        ...
        steam,
        T,
        dT)*X[Water] + myMedia.IdealGases.Common.Functions.s0_Tlow(dryair, T)*
      dX[Air] + myMedia.IdealGases.Common.Functions.s0_Tlow(steam, T)*dX[Water]
       - Modelica.Constants.R*(1/MMX[Water]*Utilities.smoothMax_der(
        ...
        (Modelica.Math.log(max(Y[Water], Modelica.Constants.eps)*p/reference_p)
         + (X[Water]/Y[Water]*(X[Air]*MMX[Water]/(X[Air]*MMX[Water] + X[Water]*
        MMX[Air])^2)))*dX[Water] + X[Water]*reference_p/p*dp,
        0,
        0) - 1/MMX[Air]*Utilities.smoothMax_der(
        (1 - X[Water])*Modelica.Math.log(max(Y[Air], Modelica.Constants.eps)*p/
        reference_p),
        0.0,
        1e-9,
        (Modelica.Math.log(max(Y[Air], Modelica.Constants.eps)*p/reference_p)
         + (X[Air]/Y[Air]*(X[Water]*MMX[Air]/(X[Air]*MMX[Water] + X[Water]*MMX[
        Air])^2)))*dX[Air] + X[Air]*reference_p/p*dp,
        0,
        0));
    annotation (
      Inline=false,
