...
model SimpleEngine "Simple steam engine"
  ...
  package Water = .ThermofluidStream.Media.myMedia.Water.StandardWater;


  inner .ThermofluidStream.DropOfCommons dropOfCommons(
    L=1e-3,
    assertionLevel = AssertionLevel.warning,displayParameters = true)
    annotation (Placement(transformation(extent={{140,-34},{160,-14}})));
  .ThermofluidStream.Examples.Utilities.SteamSink steamSink(
  ...
  .ThermofluidStream.Examples.Utilities.CrankDrive crankDrive(
    rodLength=3,
    crankPoint=0.25,
    translationMass=1e2,
    J_flyweel=1e2,
    phi0=-1.5707963267949)
    annotation (Placement(transformation(extent={{2.0,-38.0},{-118.0,-98.0}},rotation = 0.0,origin = {0.0,0.0})));

  .ThermofluidStream.Examples.Utilities.Piston piston(
    redeclare package Medium = Water,
    A=0.05,
    l_stroke=0.5,
    V_dead=0.2,
  ...
  .ThermofluidStream.Undirected.Topology.ConnectorInletOutletFore switchConnector(
    redeclare package Medium = Water) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={70,34})));
  .ThermofluidStream.Undirected.Topology.ConnectorInletOutletFore switchConnector1(
    redeclare package Medium = Water) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={50,54})));
  .ThermofluidStream.FlowControl.Switch switch(redeclare package Medium = Water) annotation (Placement(transformation(extent={{-10,10},{10,-10}},
        rotation=0,
        origin={-10,54})));
  .ThermofluidStream.Topology.JunctionT2 junctionT2_1(redeclare package Medium = Water)
  ...
  .ThermofluidStream.FlowControl.TanValve tanValve(redeclare package Medium = Water, invertInput=false) annotation (Placement(transformation(extent={{100,64},{120,44}})));
  .ThermofluidStream.FlowControl.TanValve tanValve1(redeclare package Medium = Water, invertInput=true) annotation (Placement(transformation(extent={{120,24},{140,44}})));
  ...
  .Modelica.Blocks.Continuous.FirstOrder firstOrder(T=0.01, initType=.Modelica.Blocks.Types.Init.SteadyState)
    annotation (Placement(transformation(extent={{8,8},{-8,-8}},
        rotation=180,
        origin={-40,-16})));
  .ThermofluidStream.Boundaries.PhaseSeparator boiler(
    redeclare package Medium = Water,
    useHeatport=true,
    A=20,
    p_start=400000,
  ...
  .ThermofluidStream.Sensors.SingleSensorSelect singleSensorSelect(
    ...
    filter_output=false) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-96,16})));
  .ThermofluidStream.Boundaries.TerminalSource terminalSource(redeclare package Medium = Water) annotation (Placement(transformation(extent={{-130,44},{-110,64}})));
  .ThermofluidStream.Utilities.Icons.DLRLogo dLRLogo annotation (Placement(transformation(extent={{162,-118},{218,-62}})));
    .Modelica.Blocks.Sources.Trapezoid trapezoid(period = 5,offset = 1,amplitude = 0.5,rising = 0.5,falling = 0.5,width = 1.0) annotation(Placement(transformation(extent = {{-208,-80},{-188,-60}},origin = {0,0},rotation = 0)));
    .Modelica.Mechanics.Rotational.Sources.Torque torque annotation(Placement(transformation(extent = {{-166,-78},{-146,-58}},origin = {0,0},rotation = 0)));
equation
  ...
  connect(switch.outletB, flowResistance4.inlet)
    annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(crankDrive.h_out, booleanToReal.u) annotation (Line(points={{-88,-38},{-88,-16},{-79.6,-16}}, color={255,0,255}));
  ...
  connect(crankDrive.flange_b, piston.flange) annotation (Line(points={{2,-68},{2,-70},{20,-70}},  color={0,127,0}));
    connect(trapezoid.y,torque.tau) annotation(Line(points = {{-187,-70},{-177.5,-70},{-177.5,-68},{-168,-68}},color = {0,0,127}));
    connect(torque.flange,crankDrive.flange_a) annotation(Line(points = {{-146,-68},{-88,-68}},color = {0,0,0}));
  annotation (
