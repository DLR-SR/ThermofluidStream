...
package ReferenceAir
  ...
  constant
    ThermofluidStream.Media.myMedia.Interfaces.Types.TwoPhase.FluidConstants airConstants(
...
public
  partial package Air_Base
    ...
    extends ThermofluidStream.Media.myMedia.Interfaces.PartialPureSubstance(
      mediumName="Air",
      substanceNames={"air"},
      singleState=false,
    ...
    algorithm
      MM := ThermofluidStream.Media.myMedia.Air.ReferenceAir.airConstants.molarMass;
      annotation (Inline=true);
    end molarMass;
...
<p>Many further properties can be computed. Using the well-known Bridgman's Tables, all first partial derivatives of the standard thermodynamic variables can be computed easily.</p>
  ...
  package Air_Utilities
    ...
    package Basic "Fundamental equation of state"
      ...
      constant ThermofluidStream.Media.myMedia.Common.FundamentalConstants Constants(
        final R_bar=8.31451,
        final R_s=287.117,
  ...
  protected
    package ThermoFluidSpecial
      ...
      algorithm
        d :=
          ThermofluidStream.Media.myMedia.Air.ReferenceAir.Air_Utilities.Inverses.dofpT(
                p=p,
                T=T,
                delp=1e-7);
        f := Air_Utilities.Basic.Helmholtz(d, T);
