...
partial model PartialNTU3 "Base heat exchanger using the epsilon-NTU method"
...
protected
  ThermofluidStream.Interfaces.Inlet inletA_internal(redeclare package Medium = MediumA);
  ThermofluidStream.Interfaces.Outlet outletA_internal(redeclare package Medium = MediumA);
  ThermofluidStream.Interfaces.Inlet inletB_internal(redeclare package Medium = MediumB);
  ThermofluidStream.Interfaces.Outlet outletB_internal(redeclare package Medium = MediumB);
public
...
protected
  ...
  Modelica.Units.SI.Pressure p_A=MediumA.pressure(inletA_internal.state);
  Modelica.Units.SI.Pressure p_B=MediumB.pressure(inletB_internal.state);

  MediumA.MassFraction Xi_A[MediumA.nXi]=MediumA.massFraction(inletA_internal.state);
  MediumB.MassFraction Xi_B[MediumB.nXi]=MediumB.massFraction(inletB_internal.state);
  ...
  Modelica.Units.SI.MassFlowRate m_flow_A=inletA_internal.m_flow "Mass flow on side A";
  Modelica.Units.SI.MassFlowRate m_flow_B=inletB_internal.m_flow "Mass flow on side B";

  SI.SpecificHeatCapacity cpA_in=MediumA.specificHeatCapacityCp(inletA_internal.state);
  SI.SpecificHeatCapacity cpA_out=MediumA.specificHeatCapacityCp(outletA_internal.state);
  SI.SpecificHeatCapacity cpB_in=MediumB.specificHeatCapacityCp(inletB_internal.state);
  SI.SpecificHeatCapacity cpB_out=MediumB.specificHeatCapacityCp(outletB_internal.state);
...
equation
  ...
  //Balance Equations
  inletA_internal.m_flow + outletA_internal.m_flow = 0;
  inletB_internal.m_flow + outletB_internal.m_flow = 0;
  inletA_internal.r - outletA_internal.r = der(inletA_internal.m_flow)*L;
  inletB_internal.r - outletB_internal.r = der(inletB_internal.m_flow)*L;
  ...
  //Heat capacity rates
  C_A = (abs(inletA_internal.m_flow) + eps)*cp_A;
  C_B = (abs(inletB_internal.m_flow) + eps)*cp_B;

  //Inlet temperatures and enthalpies
  T_in_MediumA = MediumA.temperature(inletA_internal.state);
  T_in_MediumB = MediumB.temperature(inletB_internal.state);

  h_in_A = MediumA.specificEnthalpy(inletA_internal.state);
  h_in_B = MediumB.specificEnthalpy(inletB_internal.state);
  ...
  if noEvent(C_A < C_B) then
    ...
    if noEvent(inletA_internal.m_flow < m_flow_reg) and noEvent(inletB_internal.m_flow < m_flow_reg) then
      dh_A = 0;
    else
  ...
  outletA_internal.state = MediumA.setState_phX(
    p_A,
    h_out_A,
    Xi_A);
  outletB_internal.state = MediumB.setState_phX(
    p_B,
    h_out_B,
    Xi_B);

  //Summary record
  summary.Tin_B = MediumB.temperature(inletB_internal.state);
  summary.Tin_A = MediumA.temperature(inletA_internal.state);
  summary.Tout_B = MediumB.temperature(outletB_internal.state);
  summary.Tout_A = MediumA.temperature(outletA_internal.state);
