...
model gammaSpec "Test model to check gamma specification"
  ...
  ThermofluidStream.Idealized.Processes.Adiabatic idealGasFixed(
    redeclare package Medium = Medium,
    redeclare model ThermodynamicModel = ThermofluidStream.Idealized.Processes.Utilities.AdiabaticThermodynamicModels.Flow.IdealGasConstantGamma (gammaSpec=ThermofluidStream.Idealized.Utilities.Types.ValueSpecification2.Fixed),
    eta_fixed=eta,
    outletSpec=ThermofluidStream.Idealized.Types.OutletSpecification.Flow.Adiabatic.PressureRatio,
    outletValueSpec=ThermofluidStream.Idealized.Utilities.Types.ValueSpecification.Prescribed,
...
equation
  connect(source.outlet, idealGasState.inlet) annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(idealGasState.outlet, sink.inlet) annotation (Line(
      points={{-40,50},{-30,50}},
      color={28,108,200},
      thickness=0.5));
  connect(idealGasState.outletSpec_prescribed, pressureRatio.y) annotation (Line(points={{-40,38},{-40,0},{-59,0}}, color={0,0,127}));
  connect(source1.outlet, idealGasFixed.inlet) annotation (Line(
      points={{30,50},{40,50}},
      color={28,108,200},
      thickness=0.5));
  connect(idealGasFixed.outlet, sink1.inlet) annotation (Line(
      points={{60,50},{70,50}},
      color={28,108,200},
      thickness=0.5));
  connect(pressureRatio.y, idealGasFixed.outletSpec_prescribed) annotation (Line(points={{-59,0},{60,0},{60,38}}, color={0,0,127}));
...
assuming ideal gas (p*v = R*T) with gamma = const vs approach assuming perfect gas (p*v = R*T, cp = const). Check different media, e.g. Argon and CO2"), Text(
  ...
  <p>
    ...
    <a href=\"modelica://ThermoFluidStreamPlus.Idealized.Processes.Utilities.AdiabaticThermodynamicModels.Flow.BaseClasses.PartialIdealGas\">AdiabaticModels.BaseClasses.PartialIdealGas</a>.
  </p>
</html>", revisions="<html>
  <ul>
    <li>
