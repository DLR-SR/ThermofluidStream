...
type Isochoric = enumeration(
  ...
  OutletTemperature) "Options for specifying the independent outlet variable of a stationary-flow representation of a isochoric periodic closed-system cycle process"
    annotation(choices(__Dymola_radioButtons=true,
        choice =ThermoFluidStreamPlus.Idealized.Types.OutletSpecification.Cycle.Isochoric.TemperatureDifference   "Temperature difference",
        choice =ThermoFluidStreamPlus.Idealized.Types.OutletSpecification.Cycle.Isochoric.OutletTemperature   "Outlet temperature"));
