  ...
  annotation(
    ...
    Icon(
      ...
      graphics={
        ...
        Polygon(
          origin={-122,-30},
          rotation = DynamicSelect(0, if E_flow_in >= 0 then 0 else 180),
          points={{-18,3},{4,3},{4,10},{18,0},{4,-10},{4,-3},{-18,-3},{-18,3}},
          fillColor = {255,170,85},
          fillPattern = DynamicSelect(FillPattern.None, if abs(E_flow_in) >= 1e-8 then FillPattern.Solid else FillPattern.None),
          pattern=LinePattern.None),
        Text(
          origin={-136,-48},
          extent={{0,0},{36,36}},
          textColor={255,170,85},
          textStyle={TextStyle.Bold},
          textString = DynamicSelect("", if abs(E_flow_in) < 1e-8 then "0" else "")),
        Polygon(visible = outputDissipation,
          origin={-25,-45},
          rotation = -90,
          points={{-18,3},{4,3},{4,10},{18,0},{4,-10},{4,-3},{-18,-3},{-18,3}},
          fillColor = {255,170,85},
          fillPattern = DynamicSelect(FillPattern.None, if abs(E_flow_dissipation) >= 1e-8 then FillPattern.Solid else FillPattern.None),
          pattern=LinePattern.None),
        Text(visible = outputDissipation,
          ...
          textColor={255,170,85},
          textStyle={TextStyle.Bold},
          textString= DynamicSelect("", if abs(E_flow_dissipation) < 1e-8 then "0" else ""))}),
    Documentation(
      info="<html>
  <p>
