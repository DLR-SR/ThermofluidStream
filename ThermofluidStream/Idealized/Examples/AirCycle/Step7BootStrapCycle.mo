...
model Step7BootStrapCycle
  ...
  .ThermofluidStream.Boundaries.Sink_m airSink(redeclare package Medium = Medium, m_flowSpec=ThermofluidStream.Types.ValueSpecification.Prescribed) annotation (Placement(transformation(extent={{100,-10},{120,10}})));
  ...
  Boundaries.MassFlowRate massFlowRateSource(redeclare package Medium = Medium, m_flowSpec=ThermofluidStream.Types.ValueSpecification.Prescribed) annotation (Placement(transformation(extent={{10,-40},{30,-20}})));
  Modelica.Blocks.Sources.Ramp massFlowRate(
    height=1,
    duration=1,
    offset=1) annotation(Placement(transformation(extent={{142,-10},{122,10}})));
  Modelica.Blocks.Sources.Ramp massFlowRateValve(
    height=2,
    duration=1,
    offset=0) annotation(Placement(transformation(extent={{-40,-56},{-20,-36}})));
...
equation
  ...
  <p>
    The compressor outlet pressure depends on the turbine power, which in turn depends on the compressor 
    outlet pressure, forming an implicit nonlinear equation loop. A suitable start value is required for 
    this equation to be solvable.
  </p>
