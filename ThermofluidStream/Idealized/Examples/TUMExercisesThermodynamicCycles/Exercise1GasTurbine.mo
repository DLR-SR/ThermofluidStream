...
model Exercise1GasTurbine "Exercise 8.1: Gas Turbine Cycle with Intercooling and Recuperation [TUM2019]"
  ...
  ThermofluidStream.Boundaries.Sink_m sink(redeclare package Medium = Medium, m_flow_fixed=5.8) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={10,70})));
  ...
  ThermofluidStream.Idealized.Processes.Isobaric recuperatorA(
    redeclare package Medium = Medium,
  ...
  ThermofluidStream.Idealized.Processes.Isobaric recuperatorB(
    redeclare package Medium = Medium,
  ...
  ThermofluidStream.Idealized.EnergyFlow.Components.FixedTransferEfficiency losses(eta=0.97) annotation (Placement(transformation(extent={{160,-40},{180,-20}})));
  ThermofluidStream.Idealized.EnergyFlow.Components.Sum shaftPower(n_in=3) annotation (Placement(transformation(extent={{120,-40},{140,-20}})));
equation
