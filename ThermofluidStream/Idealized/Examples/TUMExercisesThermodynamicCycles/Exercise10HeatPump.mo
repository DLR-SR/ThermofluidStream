...
model Exercise10HeatPump
  ...
  ThermofluidStream.Idealized.Processes.Isobaric evaporator(
    ...
    outletValueSpec=ThermofluidStream.Types.ValueSpecification.Prescribed) annotation(Placement(transformation(
        ...
        rotation=90,
        origin={-70,-10})));
  Modelica.Blocks.Sources.RealExpression h_dew(y=Refrigerant.dewEnthalpy(Refrigerant.setSat_p(1e5))) annotation(
  ...
  ThermofluidStream.Idealized.Processes.Isenthalpic highPressureValve(
    ...
    p_out_fixed=600000) annotation(Placement(transformation(
        ...
        rotation=0,
        origin={40,-40})));
  ThermofluidStream.Idealized.Components.Separator flashChamber(redeclare package Medium = Refrigerant) annotation (Placement(transformation(extent={{10,-10},{-10,10}}, rotation=270)));
  ThermofluidStream.Idealized.Boundaries.LoopBreaker highPressureLoopBreaker(
    redeclare package Medium = Refrigerant,
    ...
    thermalValueSpec=ThermofluidStream.Types.ValueSpecification.Prescribed) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={24,40})));
  Modelica.Blocks.Sources.RealExpression h_dew1(y=Refrigerant.dewEnthalpy(Refrigerant.setSat_p(6e5))) annotation(
    Placement(transformation(extent={{0,50},{20,70}})));
  ThermofluidStream.Idealized.Boundaries.LoopBreaker lowPressureLoopBreaker(
    redeclare package Medium = Refrigerant,
    p_out_fixed=100000,
    thermalSpec=ThermofluidStream.Types.ThermalSpecification.SpecificEnthalpy,
    thermalValueSpec=ThermofluidStream.Types.ValueSpecification.Prescribed) annotation (Placement(transformation(
  ...
  ThermofluidStream.Boundaries.Source
    ...
    T0_par=293.15) annotation(Placement(transformation(
        ...
        rotation=90,
        origin={130,-30})));
  ThermofluidStream.Boundaries.Sink_m sinkWater(redeclare package Medium = Water, m_flow_fixed=0.2) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={130,30})));
...
equation
  connect(evaporator.inlet, lowPressureValve.outlet) annotation(
    Line(
      points={{-70,-20},{-70,-40},{-50,-40}},
      color={28,108,200},
      thickness=0.5));
  connect(evaporator.outlet, lowPressureLoopBreaker.inlet) annotation(
    Line(
      points={{-70,0},{-70,12}},
      color={28,108,200},
      thickness=0.5));
  connect(lowPressureLoopBreaker.outlet, lowPressureCompressor.inlet) annotation(
    Line(
  ...
  connect(highPressureCompressor.outlet, condensor.inlet) annotation(
    Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(condensor.outlet, highPressureValve.inlet) annotation(
    Line(
  ...
  connect(highPressureValve.outlet, flashChamber.inletB) annotation(Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(highPressureLoopBreaker.outlet, highPressureCompressor.inlet) annotation(
    Line(
      points={{34,40},{50,40}},
      color={28,108,200},
      thickness=0.5));
