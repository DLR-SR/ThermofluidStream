...
model Exercise9TwoStageSteamTurbine "Exercise 8.9: Two-Stage Steam Turbine with Extraction for District Heating [TUM2019]"
  ...
  ThermofluidStream.Idealized.Processes.Adiabatic pump1(
    ...
    p_out_fixed=5800000) annotation(Placement(transformation(
        ...
        rotation=270,
        origin={-80,-20})));
  ThermofluidStream.Idealized.Topology.JunctionT2 mixer(displayInstanceName=true, redeclare package Medium = Medium) annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=270,
        origin={-80,10})));
  ...
  ThermofluidStream.Idealized.EnergyFlow.Sources.FixedEnergyFlow powerHighPressureTurbine(E_flow(displayUnit="kW") = -800000) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-20,-10})));
  ThermofluidStream.Idealized.EnergyFlow.Components.FixedTransferEfficiency lossesHPTShaft(eta=0.92) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-20,30})));
  ThermofluidStream.Idealized.EnergyFlow.Sources.FixedEnergyFlow powerLowPressureTurbine(E_flow(displayUnit="kW") = -200000) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={60,-10})));
  ThermofluidStream.Idealized.EnergyFlow.Components.FixedTransferEfficiency lossesLPTShaft(eta=0.92) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={60,30})));
...
equation
  connect(mixer.outlet, steamBoiler.inlet) annotation(Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(lowPressureTurbine.outlet, condensor.inlet) annotation(
    Line(
  ...
  connect(pump2.outlet, mixer.inletA) annotation(Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(heatingNetwork.outlet, pump2.inlet) annotation(
    Line(
      points={{-10,-80},{-50,-80},{-50,-30}},
      color={28,108,200},
      thickness=0.5));
  connect(condensor.outlet, pump1.inlet) annotation(
    Line(
      points={{-10,-110},{-80,-110},{-80,-30}},
      color={28,108,200},
      thickness=0.5));
  connect(loopBreaker.outlet, highPressureTurbine.inlet) annotation(
    Line(
      points={{-50,60},{-30,60}},
      color={28,108,200},
      thickness=0.5));
