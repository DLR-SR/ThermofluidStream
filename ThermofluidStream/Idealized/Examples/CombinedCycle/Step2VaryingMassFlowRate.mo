...
model Step2VaryingMassFlowRate
  ...
  ThermofluidStream.Idealized.Processes.Isobaric heatExchangerWaterSide(
    ...
    T_out_fixed=473.15) annotation(Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=90,
        origin={12,0})));
  ...
  ThermofluidStream.Idealized.Processes.Adiabatic compressor(
    ...
    p_out_fixed=2000000) annotation(Placement(transformation(extent={{-140,50},{-120,30}})));
  ThermofluidStream.Idealized.Processes.Adiabatic turbineJB(
    redeclare package Medium = Air,
    powerSignal=ThermofluidStream.Idealized.Types.EnergyFlowSignalMode.Output,
    eta_fixed=0.8,
    outletSpec=ThermofluidStream.Idealized.Types.OutletSpecification.Adiabatic.OutletPressure,
    p_out_fixed=100000) annotation(Placement(transformation(extent={{-60,50},{-40,30}})));
  ...
  .ThermofluidStream.Boundaries.Sink_m airSink(redeclare package Medium = Air, m_flowSpec=ThermofluidStream.Types.ValueSpecification.Prescribed) annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-30,-30})));
  ...
  Modelica.Blocks.Sources.Ramp massFlowRateRamp(
    height=19,
    duration=10,
    offset=1) annotation(Placement(transformation(extent={{-80,-60},{-60,-40}})));
  EnergyFlow.Components.Sum generatorPower(n_in=3) annotation(Placement(transformation(extent={{160,50},{180,70}})));
...
equation
  ...
  connect(pump.outlet, heatExchangerWaterSide.inlet) annotation(
    Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(turbineCR.outlet, cooler.inlet) annotation(
    Line(
  ...
  connect(heatExchangerWaterSide.outlet, turbineCR.inlet) annotation(
    Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(compressor.outlet,combustion. inlet) annotation(
    Line(
  ...
  connect(heatExchangerAirSide.outlet,airSink. inlet) annotation(
    Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(massFlowRateRamp.y, airSink.m_flow_prescribed) annotation(Line(points={{-59,-50},{-30,-50},{-30,-32}}, color={0,0,127}));
  ...
  <p>
    Note that for an air mass flow rate below <code>1.03 kg/s</code>, the air outlet temperature
    (<code>heatExchangerAirSide.outlet.T</code>) falls below the water inlet temperature
    (<code>heatExchangerWaterSide.inlet.T</code>). This violates the second law of thermodynamics and
