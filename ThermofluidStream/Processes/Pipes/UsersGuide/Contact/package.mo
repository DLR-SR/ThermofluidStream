within ThermofluidStream.Processes.Pipes.UsersGuide;
package Contact
  extends Modelica.Icons.Contact;

  annotation (
    Documentation(info="<html><head></head><body><h4>Authors</h4>
    <blockquote>Philipp Jordan, Peter Stein<br>
      HTWG Konstanz<br>Alfred-Wachtel-Str. 8<br>
      D-78462 Konstanz<br>
      Web: <a href=\"https://www.htwg-konstanz.de\">www.htwg-konstanz.de</a>
    </blockquote>
    
    
    <h4>Contact</h4>
    <p>Prof. Dr. Peter Stein: <a href=\"mailto:pstein@htwg-konstanz.de\">pstein@htwg-konstanz.de</a>.</p>
<p>Philipp Jordan: <a href=\"mailto:philipp.jordan@htwg-konstanz.de\">philipp.jordan@htwg-konstanz.de</a>.</p><p>

<img src=\"modelica://ThermofluidStream/Resources/Doku/Fluid_HTWG/HTWG_en_Markenzeichen_klein_pos_1C.png\" alt=\"HTWG Konstanz\" width=\"350\" height=\"100\">
</p>
</body>
</html>"));
end Contact;
