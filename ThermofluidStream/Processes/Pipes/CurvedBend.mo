...
equation
  ...
  annotation (
    ...
    Documentation(info="<html><head></head><body><p>This curved pipe bend model computes the pressure loss of the fluid depending on the massflow or the massflow depending on a given pressure difference, some medium properties and the geometry of the pipe bend.
      </p><p> Note that the <strong>results may differ from actual values</strong>. Due to <strong>complex flow behavior in pipe bends, widely applicable formulas are approximations only</strong>. If the pressure drop coefficient of a real component is known, it is recommended to <strong>calibrate the model</strong> to this value <strong>using the roughness parameter</strong>.
      ...
      </p><p>This component is an adaptation of <a href=\"modelica://Modelica.Fluid.Fittings.Bends.CurvedBend\">CurvedBend</a> by Modelica to make it compatible with ThermofludiStream library.</p>
      <p></p><p>The pipe bend component is using the partial model <a href=\"modelica://Fluid_HTWG.Interfaces.SISOFlowBend\">SISOFlowBend</a> implementing the common flow balances.  For the calculation of pressure loss the function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_DP\">dp_curvedOverall_DP</a> by Modelica is implemented. The input records <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_IN_con\">dp_curvedOverall_IN_con</a> &amp; <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_IN_var\">dp_curvedOverall_IN_var</a> are overwritten with the input parameters defining the pipe bend geometry and fluid properties. For more information on the underlying pressure loss function, <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.Bend.dp_curvedOverall\">click here</a>. To improve the accuracy when compressible media are used, center state fluid properties (mean dynamic viscosity &amp; mean density) are defined and refered to in the pressure loss function.
      </p><p>The following 3 figures, resistance coefficient charts representing the pressure loss model under common conditions are shown.
