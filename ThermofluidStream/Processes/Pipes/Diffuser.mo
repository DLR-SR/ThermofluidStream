...
model Diffuser "Pressure drop of a conical diffusor"
  ...
  extends Internal.Interfaces.SISOFlow_nonConstArea(final L=if computeL then ((d_2 - d_1)/tan(alpha/2))/(d_2^2*pi/4)
  ...
  final parameter SI.Angle alpha = if not setLength then alpha_par else 2*atan((d_2-d_1)/l_par) "Central divergence angle of diffuser walls";
  final parameter SI.Length l_dif = if not setLength then (d_2 - d_1)/tan(alpha/2) else l_par "Diffuser length";
...
protected
  ...
  final parameter Real Delta = ks/d_1 "Dimensionless pipe roughness";
...
equation
  ...
  (DP,zeta_dif,zeta_exp,zeta_fr) = Internal.BaseClasses.PressureLoss.Diffuser.dp_conicalDiffuserOverall_DP(
    m_flow,
    A_1,
    A_2,
    alpha,
