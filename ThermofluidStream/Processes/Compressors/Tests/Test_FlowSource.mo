...
the inlet the source is connected to.
  ...
  inner ThermofluidStream.DropOfCommons dropOfCommons(displayInstanceNames=true, displayParameters=true) annotation (Placement(transformation(extent={{-202,68},
            {-182,88}})));
  AllMediaBasedOnEntropy.FlowSource flowSource(
    redeclare package Medium = Medium,
    eta_is=eta,
    setpoint=ThermofluidStream.Processes.Compressors.Internal.Types.CompressorSetpointModeFlow.m_flow,
  ...
  IdealGasConstantKappa.FlowSourceIdealGas flowControlledSimpleCompressor1(
    redeclare package Medium = Medium,
    eta_is=eta,
    setpoint=ThermofluidStream.Processes.Compressors.Internal.Types.CompressorSetpointModeFlow.m_flow,
    setpointFromInput=true,
