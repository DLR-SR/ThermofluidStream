...
<p>Medium package used in the Boundary. Make shure it is the same as the one the port is connected to.</p>
  ...
  Modelica.Blocks.Interfaces.RealInput T0_var( unit = "K") = T0 if  temperatureFromInput "Temperature input connector [K]" annotation (Placement(transformation(
          extent={{-20,-20},{20,20}},
        rotation=180,
        origin={20,60}),             iconTransformation(extent={{40,40},{0,80}})));
  Modelica.Blocks.Interfaces.RealInput p0_var(unit="Pa")= p0 if pressureFromInput "Pressure input connector [Pa]" annotation (Placement(transformation(
          extent={{-20,-20},{20,20}},
        rotation=180,
        origin={20,0}),              iconTransformation(extent={{40,-20},{0,20}})));
  ...
  Modelica.Blocks.Interfaces.RealInput h0_var(unit = "J/kg")= h0 if enthalpyFromInput "Enthalpy input connector"
    annotation (Placement(transformation(extent={{-20,-20},{20,20}},
        rotation=180,
        origin={20,-60}),                                             iconTransformation(extent={{40,-80},{0,-40}})));
...
equation
  ...
  rear.state_rearwards = if not setEnthalpy then Medium.setState_pTX(p0,T0,Xi0_par) else Medium.setState_phX(p0,h0,Xi0_par);
