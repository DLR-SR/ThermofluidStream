...
<p>The <b>DropOfCommons</b> defines global parameters and settings for simulation models. This model serves several purposes: </p>
<ul>
<li>It provides <b>default physical constants</b> (e.g., gravity acceleration) and flow properties (e.g., inertance) that are used by all components in the model.</li>
<li>It defines <b>regularization parameters</b> (e.g., minimal density, minimal pressure, mass flow threshold, angular velocity, and volume damping factor) that ensure numerical stability of transient simulations. These parameters must be adjusted to match the characteristics of the simulated system, for example, if the system&rsquo;s design involves mass flows on the order of magnitude of the regularization threshold (m_flow_reg), or angular velocities near omega_reg, their values should be updated to prevent artificial inaccuracies. Detailed information on the volume damping factor is available in the <a href=\"modelica://ThermofluidStream.Boundaries.Internal.PartialVolume\">PartialVolume</a> documentation.</li>
<li>It defines <b>global assertion and error handling</b> behavior via the <span style=\"font-family: Courier New;\">assertionLevel</span> parameter.</li>
<li>It provides <b>layout and visualization settings</b> for model diagrams, such as displaying instance names and parameter values.</li>
</ul>
...
<p style=\"margin-left: 40px;\"><b><span style=\"font-family: Courier New;\">inner</span></b> ThermofluidStream.DropOfCommons dropOfCommons </p>
