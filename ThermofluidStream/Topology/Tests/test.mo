...
model test
  ...
  ThermofluidStream.Boundaries.Source source(
    outlet(m_flow(start=0,fixed=true)),redeclare package Medium = myMedia.Air.MoistAir, p0_par(displayUnit = "Pa") = 1.001)  annotation(
    Placement(visible = true, transformation(origin = {-38, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ThermofluidStream.Boundaries.Sink sink(redeclare package Medium = myMedia.Air.MoistAir, p0_par(displayUnit = "Pa") = 1)  annotation(
    Placement(visible = true, transformation(origin = {36, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Processes.FlowResistance flowResistance(redeclare package Medium = myMedia.Air.MoistAir, l = 1, redeclare function pLoss = ThermofluidStream.Processes.Internal.FlowResistance.linearQuadraticPressureLoss, r = 0.05) annotation(
