...
model TargetTest_01_NOK
  ...
  Modelica.Blocks.Noise.UniformNoise heatScenario(
    samplePeriod=60,
    y_min=10,
    y_max=95) annotation (Placement(transformation(extent={{-106,0},{-86,20}})));
  ...
  FluidPortMSLtoOutletTFS_eq fluidPortMSLtoOutletTFS_eq(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{-18,-24},{-26,-16}})));
  InletTFStoFluidPortMSL_eq inletTFStoFluidPortMSL_eq(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{10,-24},{2,-16}})));
...
equation
  ...
  connect(thermalConvectionPipe.outlet, pHEXSimplified.inletA)
    annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(thermalConvectionPipe.heatPort, prescribedTemperature.port) annotation (Line(points={{-70,10},{-72,10}}, color={191,0,0}));
  connect(flowGenerator_SF.inlet, fluidPortMSLtoOutletTFS_eq.outletTFS) annotation (Line(
      points={{-30,-20},{-24.8,-20}},
      color={28,108,200},
      thickness=0.5));
  connect(pHEXSimplified.outletA, inletTFStoFluidPortMSL_eq.inletTFS)
    annotation (Line(
      points={{14,-2.66454e-15},{14,-20},{9.6,-20}},
      color={28,108,200},
      thickness=0.5));
  connect(inletTFStoFluidPortMSL_eq.portMSL, tank.ports[1]) annotation (Line(points={{4,-20},{-9,-20}}, color={0,127,255}));
  connect(fluidPortMSLtoOutletTFS_eq.portMSL, tank.ports[2]) annotation (Line(points={{-19.8,-20},{-7,-20}}, color={0,127,255}));
  connect(prescribedTemperature.T, heatScenario.y) annotation (Line(points={{-80.8,10},{-85,10}}, color={0,0,127}));
