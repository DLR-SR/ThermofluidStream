...
model SimpleCoolingCycle_02MCV_NOK "Basic cooling cycle with a load"
extends Modelica.Icons.Example;
  ...
  ThermofluidStream.HeatExchangers.CounterFlowNTU heatExchange_CounterFlowNTU(
    ...
    A=10) annotation (Placement(transformation(extent={{86,4},{106,24}})));
  FlowControl.MCV                  mCV(
    redeclare package Medium = Medium_liquid,
    initM_flow=ThermofluidStream.Utilities.Types.InitializationMethods.state,
    massFlow_set_par=0.4)
  ...
  ThermofluidStream.Sensors.MultiSensor_Tpm multiSensor_Tpm1(
    ...
    annotation (Placement(transformation(extent={{82,20},{68,40}})));
  ThermofluidStream.Sensors.MultiSensor_Tpm multiSensor_Tpm2(
    redeclare package Medium = Medium_liquid,
    temperatureUnit="degC")
    annotation (Placement(transformation(extent={{-6,20},{-20,42}})));
  ...
  ThermofluidStream.Utilities.Icons.DLRLogo dLRLogo annotation (Placement(transformation(extent={{144,-96},{180,-60}})));
  FluidPortMSLtoOutletTFS_eq fluidPortMSLtoOutletTFS_eq(redeclare package Medium = Medium_liquid)
                                                                                           annotation (Placement(transformation(extent={{18,16},{10,24}})));
  InletTFStoFluidPortMSL_eq inletTFStoFluidPortMSL_eq(redeclare package Medium = Medium_liquid)
                                                                                         annotation (Placement(transformation(extent={{46,16},{38,24}})));
  Modelica.Fluid.Vessels.OpenTank tank(
    height(displayUnit="m") = 2,
    crossArea(displayUnit="m2") = 0.01,
    p_ambient=100000,
    level_start(displayUnit="cm") = 0.1,
    redeclare package Medium = Medium_liquid,
    use_portsData=false,
    nPorts=2) annotation (Placement(transformation(extent={{18,20},{38,40}})));
equation
  connect(multiSensor_Tpm1.inlet, heatExchange_CounterFlowNTU.outletB)
    annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(multiSensor_Tpm2.outlet, mCV.inlet) annotation (Line(
      points={{-20,20},{-26,20}},
      color={28,108,200},
      thickness=0.5));
  ...
  connect(multiSensor_Tpm3.m_flow_out, feedback.u2) annotation (Line(points={{-72,16},{-80,16},{-80,-42}},
                                  color={0,0,127}));
  ...
  connect(inletTFStoFluidPortMSL_eq.portMSL,tank. ports[1]) annotation (Line(points={{40,20},{27,20}},  color={0,127,255}));
  connect(fluidPortMSLtoOutletTFS_eq.portMSL,tank. ports[2]) annotation (Line(points={{16.2,20},{29,20}},    color={0,127,255}));
  connect(multiSensor_Tpm1.outlet, inletTFStoFluidPortMSL_eq.inletTFS) annotation (Line(
      points={{68,20},{45.6,20}},
      color={28,108,200},
      thickness=0.5));
  connect(fluidPortMSLtoOutletTFS_eq.outletTFS, multiSensor_Tpm2.inlet) annotation (Line(
      points={{11.2,20},{-6,20}},
      color={28,108,200},
      thickness=0.5));
  annotation (
...
end SimpleCoolingCycle_02MCV_NOK;
