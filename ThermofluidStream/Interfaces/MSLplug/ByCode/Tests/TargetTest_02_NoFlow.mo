...
model TargetTest_02_NoFlow
  ...
  Modelica.Blocks.Noise.UniformNoise heatScenario(
    samplePeriod=60,
    y_min=10,
    y_max=95) annotation (Placement(transformation(extent={{-114,-20},{-94,0}})));
  ...
  Modelica.Thermal.HeatTransfer.Celsius.PrescribedTemperature
                                                           prescribedTemperature
                                                                              annotation (Placement(transformation(extent={{-88,-14},{-80,-6}})));
  FluidPortMSLtoOutletTFS_eq fluidPortMSLtoOutletTFS_eq(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{-32,-44},{-40,-36}})));
  InletTFStoFluidPortMSL_eq inletTFStoFluidPortMSL_eq(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{-4,-44},{-12,-36}})));
  ...
  FlowControl.Switch switch(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{-6,26},{6,14}})));
  Topology.JunctionT2 junctionT2_1(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{0,-46},{10,-34}})));
  Sensors.SingleSensorSelect singleSensorSelect(
    redeclare package Medium = Medium,
    quantity=ThermofluidStream.Sensors.Internal.Types.Quantities.T_C,
    outputValue=true) annotation (Placement(transformation(extent={{-56,20},{-36,40}})));
...
equation
  ...
  connect(mCV.inlet, source.outlet) annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(conductionElement.heatPort, prescribedTemperature.port) annotation (Line(points={{-79.8,-10},{-80,-10}}, color={191,0,0}));
  connect(flowGenerator_SF.inlet, fluidPortMSLtoOutletTFS_eq.outletTFS) annotation (Line(
  ...
  connect(switch.outletA, pHEXSimplified.inletA) annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(inletTFStoFluidPortMSL_eq.inletTFS, junctionT2_1.outlet) annotation (Line(
      points={{-4.4,-40},{0,-40}},
      color={28,108,200},
      thickness=0.5));
  ...
  connect(fluidPortMSLtoOutletTFS_eq.portMSL, tank.ports[1]) annotation (Line(points={{-33.8,-40},{-23,-40}}, color={0,127,255}));
  connect(tank.ports[2], inletTFStoFluidPortMSL_eq.portMSL) annotation (Line(points={{-21,-40},{-10,-40}}, color={0,127,255}));
  connect(prescribedTemperature.T, heatScenario.y) annotation (Line(points={{-88.8,-10},{-93,-10}}, color={0,0,127}));
  connect(singleSensorSelect.value_out, PID.u_s) annotation (Line(points={{-36,30},{-28,30},{-28,40},{-24.8,40}}, color={0,0,127}));
  connect(stepoint.y, PID.u_m) annotation (Line(points={{-33.4,40},{-32,40},{-32,35.2},{-20,35.2}}, color={0,0,127}));
  connect(conductionElement.inlet, flowResistance.outlet) annotation (Line(
      points={{-70,-20},{-70,-24}},
      color={28,108,200},
      thickness=0.5));
  connect(flowGenerator_SF.outlet, flowResistance.inlet) annotation (Line(
      points={{-64,-40},{-70,-40},{-70,-36}},
      color={28,108,200},
      thickness=0.5));
