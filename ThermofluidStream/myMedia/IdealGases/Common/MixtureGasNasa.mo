within ThermofluidStream.myMedia.IdealGases.Common;
...
//   end FluidConstants;
  ...
  constant myMedia.IdealGases.Common.DataRecord[:] data
...
required from medium model \"" + mediumName + "\".");
  ...
  function h_TX "Return specific enthalpy"
    import ThermofluidStream.myMedia.Interfaces.Choices;
     extends Modelica.Icons.Function;
     ...
     input Boolean exclEnthForm=excludeEnthalpyOfFormation
      "If true, enthalpy of formation Hf is not included in specific enthalpy h";
     input myMedia.Interfaces.Choices.ReferenceEnthalpy refChoice=
  ...
  algorithm
    h := (if fixedX then reference_X else X)*{
      myMedia.IdealGases.Common.Functions.h_T(
        data[i],
        T,
        exclEnthForm,
        refChoice,
  ...
  algorithm
    h_der := if fixedX then dT*sum((myMedia.IdealGases.Common.Functions.cp_T(
      data[i], T)*reference_X[i]) for i in 1:nX) else dT*sum((
      myMedia.IdealGases.Common.Functions.cp_T(data[i], T)*X[i]) for i in 1:nX)
       + sum((myMedia.IdealGases.Common.Functions.h_T(data[i], T)*dX[i]) for i in
          1:nX);
  ...
  algorithm
    cp := {myMedia.IdealGases.Common.Functions.cp_T(data[i], state.T) for i in
      1:nX}*state.X;
  ...
  algorithm
    cv := {myMedia.IdealGases.Common.Functions.cp_T(data[i], state.T) for i in
      1:nX}*state.X - data.R*state.X;
    annotation(Inline=true, smoothOrder = 1);
  ...
  algorithm
    s := sum(myMedia.IdealGases.Common.Functions.s0_T(data[i], T)*X[i] for i in
          1:size(X, 1));
    annotation(Inline=true,smoothOrder=2);
  end s_TX;
...
and etai&gt;&gt;etaj.<br>
  ...
  protected
    ...
    algorithm
      for i in 1:nX loop
      etaX[i] :=
        myMedia.IdealGases.Common.Functions.dynamicViscosityLowPressure(
            state.T,
...
average of the pure component conductivities.
  ...
  protected
    ...
    assert(fluidConstants[i].hasCriticalData, "Critical data for "+ fluidConstants[i].chemicalFormula +
      ...
      lambdaX[i] :=
        myMedia.IdealGases.Common.Functions.thermalConductivityEstimate(
            Cp=cp[i],
            eta=eta[i],
            method=method,
            data=data[i]);
